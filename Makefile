...
install: ## install Bash
	...
	curl -o '$(root)/usr/src/bash51-0#1' -fsSL 'https://ftp.gnu.org/gnu/bash/bash-5.1-patches/bash51-0$(bash_patches)'
	tar fx '$(root)/usr/src/$(bash_archive)' -C '$(root)/usr/src'
	mv '$(root)'/usr/src/bash51-00? '$(root)/usr/src/$(bash)'
	cd '$(root)/usr/src/$(bash)' && find . -type f -name 'bash51-0??' -print0 | sort -nz | xargs -0 -n 1 -I % patch -p 0 -i %
